...
export const Route = createFileRoute("/trip")({
  head: () => ({
    meta: [
      { title: "旅ログ / Trip Log — YururiMap" },
      { name: "description", content: "Your mood history and map." },
    ],
  }),
  component: TripPage,
});
...
function TripPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Read-only mood counts — displayed as flat tiles (not buttons) */}
      <div className="grid grid-cols-5 gap-1.5" aria-label={t(lang, "気持ちの内訳", "Mood breakdown")}>
        {counts.map((c) => (
          <div key={c.ja} className="rounded-xl bg-card px-1.5 py-2 flex flex-col items-center min-h-[86px]">
            <span className="w-9 h-9 rounded-full flex items-center justify-center mb-1"
            ...
            <span className="text-lg font-bold tabular-nums mt-0.5 text-foreground">
              {c.count}
